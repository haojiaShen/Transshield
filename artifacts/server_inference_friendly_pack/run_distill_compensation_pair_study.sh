...
DISTILL_PROFILE="${DISTILL_PROFILE:-official}"
BASELINE_CLS_DISTILL_WEIGHT="${BASELINE_CLS_DISTILL_WEIGHT:-0.0}"
BASELINE_TOKEN_DISTILL_WEIGHT="${BASELINE_TOKEN_DISTILL_WEIGHT:-0.0}"
CANDIDATE_CLS_DISTILL_WEIGHT="${CANDIDATE_CLS_DISTILL_WEIGHT:-1.0}"
CANDIDATE_TOKEN_DISTILL_WEIGHT="${CANDIDATE_TOKEN_DISTILL_WEIGHT:-0.02}"

DEFAULT_PAIR_NAME="distill_comp_pair_epoch${PAIR_EPOCHS}_${DISTILL_PROFILE}_$(date +%Y%m%d_%H%M%S)"
PAIR_NAME="${PAIR_NAME:-$DEFAULT_PAIR_NAME}"
BASELINE_RUN_NAME="${BASELINE_RUN_NAME:-${PAIR_NAME}_nodistill}"
CANDIDATE_RUN_NAME="${CANDIDATE_RUN_NAME:-${PAIR_NAME}_${DISTILL_PROFILE}}"
PAIR_OUTPUT_DIR="${PAIR_OUTPUT_DIR:-$REPO_ROOT/results/distillation_compensation/$PAIR_NAME}"
BASELINE_RESULT_DIR="${BASELINE_RESULT_DIR:-$REPO_ROOT/results/distillation_compensation/$BASELINE_RUN_NAME}"
CANDIDATE_RESULT_DIR="${CANDIDATE_RESULT_DIR:-$REPO_ROOT/results/distillation_compensation/$CANDIDATE_RUN_NAME}"
...
BASELINE_DISTILL_JSON="$BASELINE_RESULT_DIR/distill_log_report.json"
CANDIDATE_DISTILL_JSON="$CANDIDATE_RESULT_DIR/distill_log_report.json"
PAIR_COMPARE_JSON="$PAIR_OUTPUT_DIR/distill_compensation_pair_compare.json"
PAIR_COMPARE_MD="$PAIR_OUTPUT_DIR/distill_compensation_pair_compare.md"

print_plan() {
  cat <<EOF
[distill-comp-pair] pair_name=$PAIR_NAME
[distill-comp-pair] baseline_run_name=$BASELINE_RUN_NAME
[distill-comp-pair] candidate_run_name=$CANDIDATE_RUN_NAME
[distill-comp-pair] pair_epochs=$PAIR_EPOCHS
[distill-comp-pair] distill_profile=$DISTILL_PROFILE
[distill-comp-pair] train_data_path=$TRAIN_DATA_PATH
[distill-comp-pair] val_data_path=$VAL_DATA_PATH
[distill-comp-pair] base_checkpoint=$BASE_CHECKPOINT
[distill-comp-pair] teacher_checkpoint=$TEACHER_CHECKPOINT
[distill-comp-pair] baseline_cls_distill_weight=$BASELINE_CLS_DISTILL_WEIGHT
[distill-comp-pair] baseline_token_distill_weight=$BASELINE_TOKEN_DISTILL_WEIGHT
[distill-comp-pair] candidate_cls_distill_weight=$CANDIDATE_CLS_DISTILL_WEIGHT
[distill-comp-pair] candidate_token_distill_weight=$CANDIDATE_TOKEN_DISTILL_WEIGHT
[distill-comp-pair] baseline_run_dir=$BASELINE_RUN_DIR
[distill-comp-pair] candidate_run_dir=$CANDIDATE_RUN_DIR
[distill-comp-pair] baseline_result_dir=$BASELINE_RESULT_DIR
[distill-comp-pair] candidate_result_dir=$CANDIDATE_RESULT_DIR
[distill-comp-pair] pair_output_dir=$PAIR_OUTPUT_DIR
EOF
}

ensure_common_inputs() {
  if [[ ! -f "$BASE_CHECKPOINT" ]]; then
    echo "[distill-comp-pair] missing BASE_CHECKPOINT: $BASE_CHECKPOINT" >&2
    exit 2
  fi
  if [[ ! -f "$TEACHER_CHECKPOINT" ]]; then
    echo "[distill-comp-pair] missing TEACHER_CHECKPOINT: $TEACHER_CHECKPOINT" >&2
    exit 2
  fi
  if [[ ! -d "$TRAIN_DATA_PATH" ]]; then
    echo "[distill-comp-pair] missing TRAIN_DATA_PATH: $TRAIN_DATA_PATH" >&2
    exit 2
  fi
  if [[ ! -d "$VAL_DATA_PATH" ]]; then
    echo "[distill-comp-pair] missing VAL_DATA_PATH: $VAL_DATA_PATH" >&2
    exit 2
  fi
}
...
run_baseline_train() {
  ...
  export CLS_DISTILL_WEIGHT="$BASELINE_CLS_DISTILL_WEIGHT"
  export TOKEN_DISTILL_WEIGHT="$BASELINE_TOKEN_DISTILL_WEIGHT"
  echo "[distill-comp-pair] train baseline -> $BASELINE_RUN_DIR"
...
run_candidate_train() {
  ...
  export CLS_DISTILL_WEIGHT="$CANDIDATE_CLS_DISTILL_WEIGHT"
  export TOKEN_DISTILL_WEIGHT="$CANDIDATE_TOKEN_DISTILL_WEIGHT"
  echo "[distill-comp-pair] train candidate profile=$DISTILL_PROFILE -> $CANDIDATE_RUN_DIR"
  bash "$SCRIPT_DIR/run_secure_static_distill_train.sh" epoch1 "$@"
}
...
run_post_common() {
  ...
  if [[ ! -f "$run_dir/checkpoint-best.pth" ]]; then
    echo "[distill-comp-pair] missing checkpoint-best.pth under $run_dir" >&2
    exit 2
  fi

  mkdir -p "$result_dir"
  echo "[distill-comp-pair] threshold search -> $threshold_json"
  ...
  echo "[distill-comp-pair] threshold eval -> $threshold_eval_json"
  ...
  echo "[distill-comp-pair] plaintext eval -> $plaintext_eval_json"
...
run_post_baseline() {
  ...
  echo "[distill-comp-pair] baseline distill report -> $BASELINE_DISTILL_JSON"
  "$PYTHON_BIN" tools/transshield_distill_log_report.py \
    --train-log "$BASELINE_RUN_DIR/train_stdout.log" \
    --output-json "$BASELINE_DISTILL_JSON" \
    --output-md "$BASELINE_RESULT_DIR/distill_log_report.md"
}
...
run_post_candidate() {
  ...
  echo "[distill-comp-pair] candidate distill report -> $CANDIDATE_DISTILL_JSON"
  "$PYTHON_BIN" tools/transshield_distill_log_report.py \
    --train-log "$CANDIDATE_RUN_DIR/train_stdout.log" \
    --output-json "$CANDIDATE_DISTILL_JSON" \
    --output-md "$CANDIDATE_RESULT_DIR/distill_log_report.md"
}

run_compare() {
  mkdir -p "$PAIR_OUTPUT_DIR"
  echo "[distill-comp-pair] pair compare -> $PAIR_COMPARE_JSON"
  "$PYTHON_BIN" tools/transshield_training_pair_compare.py \
    --study-kind distill_compensation \
    ...
    --baseline-distill-report-json "$BASELINE_DISTILL_JSON" \
    --candidate-distill-report-json "$CANDIDATE_DISTILL_JSON" \
