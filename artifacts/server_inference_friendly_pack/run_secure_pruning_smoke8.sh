...
RUN_NAME="${RUN_NAME:-secure_pruning_spu_smoke8_partylocal_secret_20260510_2}"
...
# All 3 manifests for party-local share load mode
SMOKE16_DIR="$REPO_ROOT/artifacts/server_pipeline_run/e2e_aanone_exactln_clip0_smoke16_20260507_1/e2e_secure_poc"
SHARE_PUBLIC_MANIFEST="${SHARE_PUBLIC_MANIFEST:-$SMOKE16_DIR/client_pixel_values_debug_share_public_manifest.json}"
P1_SHARE_MANIFEST="${P1_SHARE_MANIFEST:-$SMOKE16_DIR/client_pixel_values_debug_share_party_manifests/p1_share_manifest.json}"
P2_SHARE_MANIFEST="${P2_SHARE_MANIFEST:-$SMOKE16_DIR/client_pixel_values_debug_share_party_manifests/p2_share_manifest.json}"

MAX_SAMPLES="${MAX_SAMPLES:-8}"
SPU_BATCH_SIZE="${SPU_BATCH_SIZE:-1}"

mkdir -p "$(dirname "$LOG_FILE")"

echo "=== Secure Pruning smoke8 (PredictorLG in-SPU) ===" | tee "$LOG_FILE"
...
with open('$CANDIDATE_JSON') as f:
    d = json.load(f)
keys = ['sample_count','finite_logits','elapsed_sec','backend','forward_scope',
        'host_plaintext_pixel_values_materialized','host_model_params_materialized',
        'reveal_policy','runtime_pruning_keep_mask_pt']
for k in keys:
    if k in d:
        print(f'  {k}: {d[k]}')
