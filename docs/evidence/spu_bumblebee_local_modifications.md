...
配置文件位置：`configs/transshield_runtime/2pc.json`
...
在 `configs/transshield_runtime/2pc.json` 中设置：
...
| 运行时配置 | `configs/transshield_runtime/2pc.json` | Cheetah 2PC 运行时配置 |
| SPU forward 实现 | `integrations/transshield_runtime/e2e_secure_vit/spu_static_vit.py` | 含 PredictorLG、bitonic sort、secure pruning |
