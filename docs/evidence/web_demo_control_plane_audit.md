...
> 说明：本文档保留为正式报告的历史审计证据。文中引用的 `demo_app/` 路径对应的是上一版正式前端实现；该目录现已移除，等待新前端重建。
...
### 2.2 `demo_app/control_plane_worker.js`
...
### 3.1 `demo_app/transshield_demo_server.py`
...
- 协议 / multipart 结构模糊测试：
  - `python tools/fuzzing/protocol_fuzz.py --base-url http://127.0.0.1:7860`
- 并发守卫 / 重放 / inflight / 限频测试：
  - 推荐先以 `WEB_DEMO_TEST_ACCEPTED_SLEEP_SEC=1.5` 启动 demo server
  - 再运行 `python tools/fuzzing/guard_stress.py --base-url http://127.0.0.1:7860`
- worker 前端自测页：
