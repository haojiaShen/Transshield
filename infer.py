...
def main(args):
    ...
    validate(data_loader_val, model, criterion, binary_threshold=args.eval_binary_threshold)
...
def validate(val_loader, model, criterion, binary_threshold=None):
    ...
    with torch.no_grad():
        ...
        for i, (images, target) in enumerate(val_loader):
            ...
            if binary_threshold is not None and output.shape[-1] == 2:
                class1_prob = torch.softmax(output, dim=-1)[:, 1]
                pred = (class1_prob >= binary_threshold).long()
                acc1 = [(pred == target).float().mean() * 100.0]
                acc5 = accuracy(output, target, topk=(5,))[0:1]
            else:
