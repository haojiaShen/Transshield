# OpenBumbleBee Transshield e2e secure ViT track

This directory hosts the experimental **whole-forward e2e secure inference** path.

It is intentionally separate from the current live bridges:

- `transshield_network_kth_bridge/`
- `transshield_tie_policy_bridge/`

because the current production-like secure path in `Transshield_final` is still:
...
- `artifacts/server_inference_friendly_pack/run_e2e_secure_poc.sh`
...
- `integrations/openbumblebee/e2e_secure_vit/debug_probe.py`
...
For the privacy-boundary migration, `run_e2e_secure_poc.sh` now exposes two
separate modes:
...
The server pack now exposes this baseline as
`artifacts/server_inference_friendly_pack/run_e2e_secure_approx_deploy.sh`,
with `make-calib-pixels`, `calibrate`, `infer`, and `all` modes.
...
and the server-friendly wrapper is:
