...
def get_args_parser():
    ...
    parser.add_argument('--model', default='deit-s', choices=['deit-s'], type=str, metavar='MODEL',
                        help='Name of model to train; final-repo live training currently supports only deit-s.')
    ...
    parser.add_argument('--eval_tie_policy', type=str, default='lowest_index',
                        choices=['lowest_index', 'highest_index'],
...
def main(args):
    ...
    if args.inference_friendly_ops:
        args.use_square_gelu = True
        args.use_approx_attn = True
        args.use_mask_pruning = True
        args.eval_pruning_mode = 'compare_network_tie'
    print(args)
    ...
    if args.layer_decay != 1.0:
        raise ValueError('Final-repo live training currently only supports deit-s with layer_decay=1.0.')
    assigner = None
