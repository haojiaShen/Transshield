...
class LUTGELU8(LUTGELU):
    """LUT GELU with 8 segments - optimized for SPU (3 binary comparisons)."""
    def __init__(self):
        super().__init__(num_segments=8)


class LUTGELU4(LUTGELU):
    """LUT GELU with 4 segments - fastest for SPU (2 binary comparisons)."""
    def __init__(self):
        super().__init__(num_segments=4)
...
def get_act_layer(act_layer):
    if isinstance(act_layer, str):
        ...
        if act_layer == 'lut_gelu_32':
            return LUTGELU32
        if act_layer == 'lut_gelu_8':
            return LUTGELU8
        if act_layer == 'lut_gelu_4':
            return LUTGELU4
...
class Attention(nn.Module):
    ...
    def _approx_attention(self, attn, attn_policy=None, eps=1e-6):
        mode = self.approx_attn_mode
        if mode == 'relu':
            attn = F.relu(attn)
        elif mode in ('relu_square', 'square_relu'):
            attn = F.relu(attn)
            attn = attn * attn
    ...
    def _forward_impl(self, x, policy):
        ...
        if self.use_approx_attn:
            ...
            attn = self._approx_attention(attn, attn_policy=attn_policy)
        elif policy is None:
...
class VisionTransformerDiffPruning(nn.Module):
    ...
    def _odd_even_sort_desc(self, values):
    ...
    def _kth_threshold_compare_network(self, masked_score, keep_count):
        token_count = masked_score.shape[1]
        padded_count = self._next_power_of_two(token_count)
        ...
        sorted_values = self._odd_even_sort_desc(sortable)
        return sorted_values[:, keep_count - 1 : keep_count]

    def _select_equal_by_index(self, equal_mask, tie_keep_quota):
        ...
        if self.eval_tie_policy == 'lowest_index':
            ordered_indices = base_indices
        elif self.eval_tie_policy == 'highest_index':
            ordered_indices = torch.flip(base_indices, dims=[0])
        else:
            raise ValueError(f'Unsupported eval_tie_policy: {self.eval_tie_policy}')
    ...
    def _build_eval_keep_decision(self, score, prev_decision, keep_count):
        active_before = prev_decision.squeeze(-1) > 0
        masked_score = score.masked_fill(~active_before, float('-inf'))

        if self.eval_pruning_mode == 'topk_argsort':
            ...
            return new_mask * prev_decision

        if self.eval_pruning_mode in ('compare_network_tie', 'f_less_tie'):
            kth_threshold = self._kth_threshold_compare_network(masked_score, keep_count)
            ...
            selected_equal_mask = self._select_equal_by_index(equal_mask, tie_keep_quota)
            keep_mask = greater_mask | selected_equal_mask
            return keep_mask.unsqueeze(-1).to(dtype=prev_decision.dtype)

        raise ValueError(f'Unsupported eval_pruning_mode: {self.eval_pruning_mode}')
        ...
        for i, blk in enumerate(list(self.blocks)[:block_limit]):
            if (not secure_static_mode or not self.secure_static_skip_pruning) and i in self.pruning_loc:
                ...
                if self.training:
                    ...
                    prev_decision = self._build_eval_keep_decision(score, prev_decision, num_keep_node)
