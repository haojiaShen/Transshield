...
export STATE_JSON="$RUN_ROOT/spu_runtime_state_medical4_all.json"
export NODE_LOG_DIR="$RUN_ROOT/node_logs_medical4_all"
cd "$REPO_ROOT"
...
stop_nodes() {
  "$PYTHON_BIN" tools/transshield_spu_runtime_setup.py stop \
    ...
    >"$RUN_ROOT/setup_medical4_all_stop.log" 2>&1 || true
}
...
"$PYTHON_BIN" tools/transshield_spu_runtime_setup.py start \
  ...
  --restart \
  | tee "$RUN_ROOT/setup_medical4_all_start.log"
...
"$PYTHON_BIN" tools/report_vps_test.py network-snapshot \
  --interface lo \
  --out "$RUN_ROOT/medical4_all.network.before.json"

/usr/bin/time -v -o "$RUN_ROOT/medical4_all.time.log" \
  "$PYTHON_BIN" \
  ...
  --output-pt "$RUN_ROOT/medical4_all.pt" \
  --output-json "$RUN_ROOT/medical4_all.json" \
  --config "$CONFIG" \
  --device cpu \
  --max-samples 4 \
  ...
  --spu-compile-cache-dir "$RUN_ROOT/compile_cache_medical4_all" \
  --token-ratio-base-override 0.655 \
  --spu-layer-norm-affine-fusion all \
  2>"$RUN_ROOT/medical4_all.stderr.log" \
  | tee "$RUN_ROOT/medical4_all.stdout.log"

"$PYTHON_BIN" tools/report_vps_test.py network-snapshot \
  --interface lo \
  --out "$RUN_ROOT/medical4_all.network.after.json"

"$PYTHON_BIN" tools/report_vps_test.py summarize \
  --dataset-key medical_secure_deployment_batch \
  --candidate-pt "$RUN_ROOT/medical4_all.pt" \
  --candidate-json "$RUN_ROOT/medical4_all.json" \
  ...
  --network-before "$RUN_ROOT/medical4_all.network.before.json" \
  --network-after "$RUN_ROOT/medical4_all.network.after.json" \
  --out "$RUN_ROOT/medical4_all.summary.json" \
  | tee "$RUN_ROOT/medical4_all.summarize.log"
