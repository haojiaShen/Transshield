...
inline void DumpCompareMeta(const std::shared_ptr<BasicOTProtocols>& base,
                            const std::string& op,
  ...
  ofs << "{"
      ...
      << "\"inp_numel\":" << inp.numel() << ","
      << "\"logical_numel\":" << numelt << ","
      << "\"bitwidth\":" << bitwidth << ","
      << "\"batch_size\":" << batch_size << ","
      << "\"compare_radix\":" << compare_radix << ","
      << "\"greater_than\":" << (greater_than ? "true" : "false") << ","
      << "\"time_ms\":" << time_ms
      << "}\n";
}
...
class ScopedCompareTimer {
 public:
  ScopedCompareTimer(const std::shared_ptr<BasicOTProtocols>& base,
                     const std::string& op,
                     const NdArrayRef& inp,
                     int64_t numelt,
                     int64_t bitwidth,
  ...
  ~ScopedCompareTimer() {
    ...
    double time_ms =
        std::chrono::duration_cast<std::chrono::duration<double, std::milli>>(
            ...
            .count();
    DumpCompareMeta(base_, op_, inp_, numelt_, bitwidth_, batch_size_,
                    compare_radix_, greater_than_, time_ms);
  }

 private:
  std::shared_ptr<BasicOTProtocols> base_;
  std::string op_;
  NdArrayRef inp_;
  int64_t numelt_;
  int64_t bitwidth_;
...
NdArrayRef CompareProtocol::DoBatchCompute(const NdArrayRef& inp,
                                           ...
                                           int64_t bitwidth,
                                           int64_t batch_size) {
  ScopedCompareTimer timer(basic_ot_prot_, "BatchCompare", inp, numelt,
                           bitwidth, batch_size, compare_radix_,
                           greater_than);
