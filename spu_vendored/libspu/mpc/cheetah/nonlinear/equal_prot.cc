...
inline bool DumpEnabled() {
  const char* env = std::getenv("SPU_DUMP_PROTOCOLS");
  return env != nullptr && std::string(env) == "1";
}

inline std::string DumpDir() {
  const char* env = std::getenv("SPU_DUMP_DIR");
  return env != nullptr ? std::string(env) : "/tmp/spu_dump";
}

inline std::string DumpReqId() {
  const char* env = std::getenv("SPU_DUMP_REQ_ID");
  return env != nullptr ? std::string(env) : "default_req";
}

inline std::string ShapeToJson(const Shape& shape) {
...
inline void DumpEqualMeta(const std::shared_ptr<BasicOTProtocols>& base,
                          const std::string& op,
                          ...
                          int64_t radix,
                          int64_t num_digits,
                          int64_t leaf_ot_instances,
                          int64_t tree_and_rounds,
                          double leaf_ot_time_ms,
                          double tree_and_time_ms,
  ...
  path << DumpDir() << "/"
       << DumpReqId() << "_party" << rank
       << "_equal.jsonl";

  std::ofstream ofs(path.str(), std::ios::app);
  ...
  ofs << "{"
      ...
      << "\"party\":" << rank << ","
      << "\"op\":\"" << op << "\","
      << "\"shape\":" << ShapeToJson(inp.shape()) << ","
      << "\"numel\":" << inp.numel() << ","
      << "\"bit_width\":" << bit_width << ","
      << "\"compare_radix\":" << compare_radix << ","
      << "\"radix\":" << radix << ","
      << "\"num_digits\":" << num_digits << ","
      << "\"leaf_ot_instances\":" << leaf_ot_instances << ","
      << "\"tree_and_rounds\":" << tree_and_rounds << ","
      << "\"leaf_ot_time_ms\":" << leaf_ot_time_ms << ","
      << "\"tree_and_time_ms\":" << tree_and_time_ms << ","
...
NdArrayRef EqualProtocol::DoCompute(const NdArrayRef& inp, size_t bit_width) {
  ...
  int64_t leaf_ot_instances = num_cmp * num_digits;
  int64_t tree_and_rounds = 0;

  double leaf_ot_time_ms = 0.0;
  double tree_and_time_ms = 0.0;
  ...
  // Phase 1: leaf OT
  // ---------------------------
  auto leaf_ot_begin = std::chrono::steady_clock::now();
  ...
  auto leaf_ot_end = std::chrono::steady_clock::now();
  leaf_ot_time_ms =
      std::chrono::duration_cast<std::chrono::duration<double, std::milli>>(
          leaf_ot_end - leaf_ot_begin)
          .count();
  ...
  // ---------------------------
  // Phase 2: tree AND reduction
  // ---------------------------
  auto tree_and_begin = std::chrono::steady_clock::now();
  ...
  auto tree_and_end = std::chrono::steady_clock::now();
  tree_and_time_ms =
      std::chrono::duration_cast<std::chrono::duration<double, std::milli>>(
          tree_and_end - tree_and_begin)
          .count();
  ...
  DumpEqualMeta(basic_ot_prot_, "Equal", inp, bit_width, compare_radix_, radix,
                num_digits, leaf_ot_instances, tree_and_rounds,
                leaf_ot_time_ms, tree_and_time_ms, total_time_ms);
