...
inline std::string ShapeToJson(const NdArrayRef& arr) {
...
inline void DumpProtocolMeta(const std::string& op,
                             const std::shared_ptr<Communicator>& conn,
                             const NdArrayRef& arr,
                             double time_ms) {
  ...
  ofs << "{"
      ...
      << "\"party\":" << rank << ","
      << "\"op\":\"" << op << "\","
      << "\"shape\":" << ShapeToJson(arr) << ","
      << "\"numel\":" << arr.numel() << ","
      << "\"dtype\":\"" << arr.eltype().toString() << "\","
      << "\"time_ms\":" << time_ms
      << "}\n";
}

class ScopedProtocolTimer {
 public:
  ScopedProtocolTimer(const std::string& op,
                      const std::shared_ptr<Communicator>& conn,
                      const NdArrayRef& arr)
      : op_(op), conn_(conn), arr_(arr),
  ...
  ~ScopedProtocolTimer() {
    ...
    double time_ms =
        std::chrono::duration_cast<std::chrono::duration<double, std::milli>>(
            ...
            .count();
    DumpProtocolMeta(op_, conn_, arr_, time_ms);
  }
...
NdArrayRef BasicOTProtocols::Multiplexer(const NdArrayRef &msg,
  ...
  ScopedProtocolTimer timer("Multiplexer", conn_, msg);
