...
E2E_INPUT_SIZE = 224
E2E_CHANNELS = 3
E2E_FLOAT_COUNT = E2E_CHANNELS * E2E_INPUT_SIZE * E2E_INPUT_SIZE
E2E_SHARE_BYTE_COUNT = E2E_FLOAT_COUNT * 4
...
def make_synthetic_rgb(tensor_seed: int = 0) -> np.ndarray:
    rng = np.random.default_rng(tensor_seed)
    xs = np.linspace(0.0, 2.0 * np.pi, E2E_INPUT_SIZE, dtype=np.float32)
    ys = np.linspace(0.0, 2.0 * np.pi, E2E_INPUT_SIZE, dtype=np.float32)
    grid_x, grid_y = np.meshgrid(xs, ys)
    base = 0.5 + 0.18 * np.sin(grid_x * 2.7) * np.cos(grid_y * 3.1)
    ripple = 0.08 * np.sin(grid_x * 7.0 + 0.3) + 0.05 * np.cos(grid_y * 5.0 - 0.7)
    noise = rng.normal(0.0, 0.01, size=(E2E_INPUT_SIZE, E2E_INPUT_SIZE)).astype(np.float32)
    ...
    rgb = np.stack([red, green, blue], axis=0)[None, ...].astype(np.float32)
    return rgb


def make_synthetic_tensor(tensor_seed: int = 0) -> np.ndarray:
    rgb = make_synthetic_rgb(tensor_seed)
    return ((rgb - IMAGENET_MEAN) / IMAGENET_STD).astype(np.float32)
...
def build_medical_parts(
    ...
    tensor = make_synthetic_tensor(tensor_seed)
    rgb = tensor * IMAGENET_STD + IMAGENET_MEAN
    ...
    source_image_sha256 = hashlib.sha256(f'synthetic-source:{tensor_seed}'.encode('utf-8')).hexdigest()
    audit_nonce = nonce or str(uuid.uuid4())
    audit_chain_sha256 = hashlib.sha256(
        (
            f'v7|{audit_nonce}|{source_image_sha256}|{normalized_tensor_sha256}|'
            f'{share0_sha256}|{share1_sha256}'
        ).encode('utf-8')
    ).hexdigest()
    quality_summary = quality_override or compute_quality_summary(rgb)
    ...
    return [
        ...
        },
        {
            ...
            'body': MEDICAL_CONTROL_PLANE_CONTRACT_VERSION.encode('utf-8'),
        },
        {
            'name': 'client_quality_summary',
            'body': json.dumps(quality_summary, ensure_ascii=False, separators=(',', ':')).encode('utf-8'),
            'content_type': 'application/json',
        },
        {
            'name': 'client_audit_manifest',
            'body': json.dumps(
                {
        ...
        {
            'name': 'client_control_plane_metrics',
...
def build_finance_parts(sample_id: str):
    return [
        {'name': 'domain', 'body': b'finance'},
        {'name': 'sample_id', 'body': sample_id.encode('utf-8')},
    ]
