...
class VisionTransformerDiffPruning(nn.Module):
    ...
    def _debug_log_keep_diagnostics(self, predictor_index, pred_score, prev_decision,
                                    raw_keep_decision, final_keep_decision):
        if not self.debug_nan or not self.training:
            return
        if predictor_index not in {1, 2}:
            return
        step = self._debug_step()

        active_mask = prev_decision.squeeze(-1).detach() > 0
        active_counts = active_mask.float().sum(dim=1)
        raw_counts = raw_keep_decision.detach().float().sum(dim=1).squeeze(-1)
        final_counts = final_keep_decision.detach().float().sum(dim=1).squeeze(-1)

        raw_empty = int((raw_counts <= 0).sum().item())
        final_empty = int((final_counts <= 0).sum().item())
        raw_le1 = int((raw_counts <= 1).sum().item())
        final_le1 = int((final_counts <= 1).sum().item())
        ...
        margin = (pred_score[:, :, 0] - pred_score[:, :, 1]).detach().float()
        active_margin = margin[active_mask]
        abs_active_margin = active_margin.abs()

        safe_active_counts = active_counts.clamp_min(1.0)
        raw_keep_ratio = raw_counts / safe_active_counts
        final_keep_ratio = final_counts / safe_active_counts

        print(
            f"[NaNDebug][{self.debug_context}] "
            f"module=VisionTransformerDiffPruning "
            f"predictor_{predictor_index}_keep_diag "
            f"{self._debug_vector_stats('active_count', active_counts)} "
            f"{self._debug_vector_stats('raw_keep_count', raw_counts)} "
            f"{self._debug_vector_stats('final_keep_count', final_counts)} "
            f"raw_empty={raw_empty} final_empty={final_empty} "
            f"raw_le1={raw_le1} final_le1={final_le1} "
            f"{self._debug_vector_stats('raw_keep_ratio', raw_keep_ratio)} "
            f"{self._debug_vector_stats('final_keep_ratio', final_keep_ratio)} "
            f"{self._debug_vector_stats('active_margin', active_margin)} "
            f"{self._debug_vector_stats('active_abs_margin', abs_active_margin)}"
        )
    ...
    def forward(self, x):
        ...
        for i, blk in enumerate(self.blocks):
            if i in self.pruning_loc:
                ...
                if self.training:
                    raw_keep_decision = F.gumbel_softmax(pred_score, hard=True)[:, :, 0:1] * prev_decision
                    hard_keep_decision = self._ensure_non_empty_keep_decision(
                        raw_keep_decision, pred_score, prev_decision, p_count
                    )
                    self._debug_log_keep_diagnostics(
                        p_count, pred_score, prev_decision, raw_keep_decision, hard_keep_decision
                    )
